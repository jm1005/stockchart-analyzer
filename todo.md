...
## Phase 16: 제스처 기반 차트 상호작용 완전 재설계
- [x] 버튼 기반 줌 UI 제거 (줌 레벨 표시, +/- 버튼)
- [x] 연속적 핀치 제스처 구현 (기중점 기반)
- [x] 팬 제스처 최적화 (좋우 스크롤, 바운더리 처리)
- [x] 데이터 윈도우 계산 (startIndex, endIndex 실시간 업데이트)
- [x] Y축 동적 스케일 (화면 범위의 최고가/최저가 기반)
- [x] 캠들 가상화 (100~200개 제한, 나머지는 렌더링 제외)
- [x] useAnimatedProps 활용한 성능 최적화
- [x] 좋우 바운더리 리바운드 효과
