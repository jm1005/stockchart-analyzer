...
interface CandlestickChartWithDragProps {
  candles: Candle[];
...
export function CandlestickChartWithDrag({
  candles,
  supportResistance = [],
  patterns = [],
  indicators,
  ...
  const handlePanGesture = (event: any) => {
    if (zoomLevel <= 1) return; // 줌이 1x일 때는 드래그 불가
    
    const { translationX, velocityX } = event.nativeEvent;
    dragVelocity.current = velocityX;
    
    // 드래그 거리를 스크롤 오프셋으로 변환
  ...
  const handlePanEnd = (event: any) => {
    ...
    // 관성 스크롤: 속도가 있으면 추가로 스크롤
    if (Math.abs(velocityX) > 100) {
      const pixelsPerCandle = chartWidth / visibleCandles.length;
      const additionalOffset = (velocityX / 1000) * 5; // 속도를 오프셋으로 변환
